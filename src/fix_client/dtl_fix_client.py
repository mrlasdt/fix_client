...
from ..common.dto import OrderCandidate, OrderInFlight, OrderType
    ...
    def onExecutionReport(self, message, sessionID):
        ...
        if msgType == "0":  # new order
            ...
            created_at = datetime.strptime(
                message.getHeader().getField(52), DTLFixClient.DT_FORMAT)
            self.order_tracker.start_track_order(order_id, OrderInFlight(
                order_assigned_id, created_at, order_type, order_side, amount, price, symbol))
        elif msgType in ["1", "2"]:  # partially filled or filled
            amount = float(message.getField(32))
            price = float(message.getField(31))
            symbol = message.getField(55)
            order_side = message.getField(54)
            self._data_receive.append({"order_id": order_id, "symbol": symbol,
                                       "order_side": order_side, "amount": amount, "price": price})
            self.order_tracker.adjust_tracked_order_on_partially_filled(
                order_id, amount)
            if msgType == "2":
